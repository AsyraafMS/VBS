...
public class mainVBS extends Database {
    ...
    public static void bookvenue(Connection conn, Venue venue, Scanner input, int userID) {
        ...
        System.out.print("Are you sure you want to book this venue? (yes/no): ");
        String confirmation = input.next().toLowerCase();
        if (confirmation.equals("yes")) {
    ...
    public static void cancelBook(Connection conn, int userID, Scanner input, Venue venue) {
        ...
        if (confirmation.equals("yes")) {
            ...
            System.out.println("Cancellation action aborted.");
        }
    }
    ...
    public static void updateBook(Connection conn, Scanner input, Venue venue, int userID) {
        ...
        System.out.print("Are you sure you want to update this booking? (yes/no): ");
        String confirmation = input.next().toLowerCase();
        if (confirmation.equals("yes")) {
            venue.updateBooking(conn, userID, bookID, venueID, dateTime);
        } else {
            System.out.println("Update action cancelled.");
        }
    }
    ...
    public static void addVenue(Connection conn, Scanner input, Venue venue) {
        while (true) {
            try {
                ...
                System.out.print("Are you sure you want to add this venue? (yes/no): ");
                String confirmation = input.next().toLowerCase();
                if (confirmation.equals("yes")) {
                    venue.addVenue(conn, building, code, nameVenue, sizeVenue, typeVenue);
                    break;
                } else {
                    System.out.println("Add venue action cancelled.");
                }
    ...
    public static void updateVenue(Connection conn, Scanner input, Venue venue) {
        ...
        System.out.print("Are you sure you want to update this venue? (yes/no): ");
        String confirmation = input.next().toLowerCase();
        if (confirmation.equals("yes")) {
            venue.updateVenue(conn, venueID, new_size);
        } else {
            System.out.println("Update venue action cancelled.");
        }
    }
    ...
    public static void deleteVenue(Connection conn, Scanner input, Venue venue) {
        ...
        System.out.print("Are you sure you want to delete this venue? (yes/no): ");
        String confirmation = input.next().toLowerCase();
        if (confirmation.equals("yes")) {
            venue.deleteVenue(conn, venueID);
        } else {
            System.out.println("Delete venue action cancelled.");
        }
    }
            ...
            switch (option) {
                case 1:
                    while (true) {
                        ...
                        if (session == null) {
                            break; // Return to main menu if login fails
                        }
                        if (session[0] != 0) {
                            break;
                        }
